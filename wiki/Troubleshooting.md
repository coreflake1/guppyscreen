...
| Installer prints `This host is known to reject this device's wget over TLS — using curl directly...` | Not an error — some networks/DNS setups cause the printer's built-in `wget` to fail a TLS handshake against GitHub specifically. The installer detects this and switches to `curl` automatically; nothing for you to do. |
